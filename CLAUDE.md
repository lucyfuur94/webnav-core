...
- **`use`** — what an agent does at runtime: the playwright browser-driving primitives (`navigate`, `click`, `type`, `snapshot`, `read`, `eval`, `network`, `wait-for`, `go-back`/`reload`), the multi-step `walk`/`walk-resume`, and open-web `search`. "Use webnav" = drive the browser + walk a built map. (The internet-graph query verbs `recall`/`route`/`hop`/`locate`/`list-goals` were REMOVED from the CLI 2026-06-13; the engine modules + the `Goal` type + their tests were then DELETED from the tree 2026-06-13 — they were inert on the default saucedemo seed and contradicted the advertised surface. The v1 proof-of-engine lives in git history if that surface ever returns.)
...
3. **Store what doesn't change; cache what does.** A route has two layers:
   - **Durable element fingerprint** — each navigate/input affordance carries `elementFp: {role, name, near}` (built 2026-06-13, `2026-06-13-element-fingerprint-design.md`). role+accessible-name disambiguates heading-vs-button; `near` (a CONTENT anchor — a distinguishing text in the target's enclosing row/card, NEVER positional) disambiguates identical siblings (e.g. 50 icon buttons in a table). `deriveNear` picks the most durable anchor (id-like over free-text). Resolution is `resolveByFingerprint` (layered, deterministic, zero-LLM); recording auto-captures the fp (`recoverFingerprint` on `use click`); `dev verify --session` checks authored fps resolve uniquely against a live page. Survives redesigns; falls back to the legacy name-only `resolveStep` when no fp.
   - **Disposable selector cache** — the concrete selectors/refs that worked last time. Expected to break.
   On a walk: resolve by fingerprint (with a bounded readiness retry for JS-render races); on genuine drift, escalate to the agent for the ref and **write the repair back** as a fingerprint onto the affordance (heal → `recordElementFp`). The map **self-heals from usage**, one step at a time. Truly-identical siblings with no distinguishing text honestly escalate (never a wrong-click).
...
**Snapshot (2026-07-11): structure-inference shipped — observation-based recording→graph, validated on 4 sites (see STATUS.md top entry). Earlier snapshot (2026-06-13):** All work is **merged to `main`** in the public repo (`github.com/lucyfuur94/webnav-core`, CI: typecheck + units on Node 18/20), **build green, full unit suite passing**. webnav is a working zero-LLM CLI **and MCP server** (`webnav mcp`). Consumer verbs: `walk`/`walk-resume`/`creds`, `read`/`search`, `eval`/`network`/`go-back`/`reload`, and the `use` browser primitives (`navigate`/`snapshot`/`click`/`type`); dev verbs: the record→analyse→edit mapping flow (`record-start`/`record-stop`/`graph-analyse [--draft]`/`graph-edit`/`graph-show`/`effects`), `node-add`/`edge-add`/`node-clear`/`node-rm`/`list`/`capture`, `export-map`/`outline`/`mermaid`/`dashboard`/`sessions`/`mcp`. Default seed = saucedemo only; the website/hosted backend was split into the separate `webnav-site` repo (2026-06-10). **2026-06-13 cleanup: the parked GitHub-recall / internet-graph / goals engine (incl. the `Goal` type) was DELETED from the tree** (dead — no live verb reached it); `list` was fixed to read the store, `describe` removed (redundant with `graph-show`/`outline`). **Learning-the-core (2026-06-13):** Layer 1 — the draft now captures the full in-page affordance repertoire (mutate/reveal/input, not just navigate); Layer 2 — states carry a declared domain SHADOW (table columns/filters/createsEntity/sub-tabs) as judgment-free evidence. Both proven on a Haiku-built OrangeHRM map (learn AND use verified live).
