...
// template: their per-widget instance titles (`heading:Testuser`, `heading:Overview Dashboard`)
...
// (`heading:Testuser`, `heading:Deal Performance Dashboard`). Same instance-data prior
...
export function draftFromEffects(effects: StoredActionEffect[], packs: PatternPack[] = loadPatternPacks()): DraftGraph {
  ...
  for (const [, group] of byLabelGroups) {
    ...
    for (const p of group) {
      // param page → tab-only discriminator (a heading here is instance data: `dashboard-testuser`,
