...
export function stablePathKey(url: string): string {
  ...
  // drop trailing id-ish segments so /report/7001/<hash> → /report, /dashboard/8001 → /dashboard.
...
export function draftFromEffects(effects: StoredActionEffect[]): DraftGraph {
  ...
  // (Verified on the analytics SPA: sidebar links show on 8/9 pages; "Sales Report" on 1 — clean split.)
