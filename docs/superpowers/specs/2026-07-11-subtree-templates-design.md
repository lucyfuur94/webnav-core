...
User (2026-07-11): *"say this is for progneo, some other website comes with some other structure, we keep on supporting something or the other and updating code — how is this sustainable?"*
...
- progneo: two dashboards stayed separate provisional states — each face is dominated by its user-configured widgets, so page-level identity can't see the shared viewer template (Setup/Customize/Outline/date-range + per-widget controls).
...
1. progneo dashboards 1210 + 1215 merge into ONE confirmed viewer state; per-widget controls appear once as widget-scoped repertoire; instance titles nowhere in fingerprint/labels.
