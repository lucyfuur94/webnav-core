...
- **URL axis:** base `v3/9999` inferred by constancy; ghost `/v3/report/list` merged automatically; templates `/{x}/list`, `/report/7001/{viz}` found; `/dashboard/{param}` over-merge correctly *rejected* by the structural check (44-node list face vs viewer face).
