...
|---|---|
| `report-list-2` ghost state (progneo) | `stablePathKey` keeps version/tenant segments → pre-redirect `/v3/report/list` ≠ `/v3/1041/report/list`, name collision suffixed |
| OrangeHRM hub-and-spoke mesh (memory) | same class: pre/post-redirect URL mismatch, no alias concept |
| `Publisher`/`Country`/`02 Jul 2026`/`X Remove` stored as affordances; 28 dimension names in `shadow.filters` (progneo) | mutation-after snapshots unioned into the page's nodes; interior-synthesis + shadow read the union with no overlay/data concept |
| `dashboard` state fingerprint = `heading:Testuser` (the logged-in user's name!) | one-shot fingerprint from a single landing — instance data as state identity |
| dashboard names (`ASJDH`, `Test_demo`) inside the dashboard-list draft | single landing = no variance signal, trusted anyway |
...
1. **Settledness (time).** A state's face is a **settled landing** snapshot (readiness-gated). A navigation records its full URL chain: `requested → settled`; every non-settled URL becomes an **alias** of the settled state, never a state. Overlay open/close cycles within a page are repertoire evidence, never states. *(Kills: redirect ghosts — progneo report-list-2 AND OrangeHRM's mesh gap, one mechanism.)*
...
4. **Cross-visit / cross-instance variance (template core).** A state's durable face = what **repeats across its settled landings** (majority-presence k-of-n, not strict intersection, to tolerate A/B noise); the varying remainder = data slots, never stored. URL segments constant across the whole site = base (`v3/1041`); constant within a template = identity; varying = `{param}`. **Identity is joint**: URL-template groupings *propose*, structural similarity *disposes* (so `/dashboard/list` vs `/dashboard/1210` stay separate despite `/dashboard/{param}`; SPA same-URL states split on structure; the three `/report/16116/{viz}` variants merge). **With one observation the state is `provisional`** — kept, marked, and the map *reports what would confirm it* ("visit a second report"). Priors (a digit/hex segment is probably a param; a heading on a parameterized page is probably data) are allowed only as *provisional* markings that evidence later confirms or refutes — never silent truth. *(Kills: `heading:Testuser` fingerprints, dashboard-name leakage — proven: 2-landing `download-list` core is clean, 1-landing `dashboard-list` isn't → provisionality is the honest difference.)*
...
## Prototype evidence (real progneo effects, 185 observations, zero site rules)

- **URL axis:** base `v3/1041` inferred by constancy; ghost `/v3/report/list` merged automatically; templates `/{x}/list`, `/report/16116/{viz}` found; `/dashboard/{param}` over-merge correctly *rejected* by the structural check (44-node list face vs viewer face).
...
- **walk**: resolves against template-core fingerprints (strictly more reliable — no `Testuser`); row-scoped affordances require an anchor at pause time (existing escalation protocol, unchanged).
...
1. **progneo**, rebuilt from the same 5 approved sessions: no ghost states; no dimension/metric/date/option values anywhere in states or shadow; shell extracted (≈13 nodes); `report` ≤ ~25 page-level affordances + parameterized row templates; no fingerprint contains instance data; names clean without numeric suffixes.
...
- **B — draft rewrite** (five-axis inference + tests; fully verifiable OFFLINE against the stored progneo effects — no browser needed).
