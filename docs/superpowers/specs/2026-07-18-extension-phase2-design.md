...
| **Agent loop** | Claude **Agent SDK** (`@anthropic-ai/claude-agent-sdk`), in-process in the local Node CLI (NOT `claude -p` shelling — this loop is live/bidirectional through a real browser, not review.ts's one-shot audit; typed `tool()` defs beat reparsing stdout). **GATE RESOLVED → SDK loop (spike 2026-07-18, PASS).** A plain `node probe.mjs` (no `ANTHROPIC_API_KEY`) imported the SDK, ran `query()`, fired a custom MCP `tool()`, and returned `subtype:'success'` in ~15s — subscription login inherited from the Claude Code Keychain with zero API key. `claude -p` + MCP-stdio (`webnav mcp`) stays as the documented fallback but is not needed. |
...
## Ordered increments
0. **SPIKE — DONE (2026-07-18, PASS):** Agent-SDK auth from a plain Node process on the Claude Code subscription. Verified against `@anthropic-ai/claude-agent-sdk@0.3.214`: `query()` + `createSdkMcpServer({tools:[tool(...)]})` → `options.mcpServers` map, `allowedTools:['mcp__<server>__<tool>']` (MCP tools are namespaced), final answer = `SDKResultSuccess.result`. Ran with `ANTHROPIC_API_KEY` unset → success, tool fired, no denials. Wiring notes carry into increment 1 verbatim. **Fork resolved → SDK loop.**
...
- ~~**SDK-auth-from-Node is the one load-bearing unknown**~~ — RESOLVED, spike PASSED (increment 0, 2026-07-18). SDK loop is the path.
