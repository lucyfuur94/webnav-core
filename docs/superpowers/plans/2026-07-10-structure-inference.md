...
describe('core guidelines (settled 2026-07-10)', () => {
  ...
  it('no site-specific names in src/', () => {
    const SITES = /progneo|orangehrm|analytics\.mn|automationexercise/i;
...
it('navigate: settles (retries a loading snapshot) and records requestedUrl', async () => {
  ...
  const deps = makeDeps({   // reuse the file's existing deps builder
    ...
    currentUrl: async () => 'https://x.test/v3/1041/report/list',
  });
...
it('merges a pre-redirect ghost via requestedUrl alias + base inference (no report-list-2)', () => {
  const effects = [
    nav('https://x.test/v3/1041/dashboard/list', LIST_SNAP),          // helper: navigated effect
    nav('https://x.test/v3/1041/announcements', ANN_SNAP),
    nav('https://x.test/v3/1041/help-center', HELP_SNAP),
    nav('https://x.test/v3/1041/download/list', DL_SNAP),
    { ...nav('https://x.test/v3/1041/report/list', REPORTS_SNAP), requestedUrl: 'https://x.test/v3/report/list' },
  ];
...
it('fingerprint never contains a token absent from the multi-landing core (no user-name identity)', () => {
  // two landings of /dashboard/7: same skeleton, DIFFERENT big heading (Testuser vs Q3 Board)
...
### Task 15: offline acceptance on real progneo data (executor: opus)

**Files:** Create `tests/acceptance/progneo-local.test.ts`.
...
it('progneo acceptance: the real 5-session map is clean', () => {
...
- [ ] **Step 1:** Write + run: `npx vitest run tests/acceptance/progneo-local.test.ts` → investigate every failure as a REAL finding (fix in the producing task's code, never relax an assertion without recording why in the commit body).
- [ ] **Step 2:** Full `npm test` + `npx tsc --noEmit` green.
- [ ] **Step 3:** Rebuild the live map: `webnav dev node-clear --node progneo.analytics.mn` then `webnav dev graph-analyse --host progneo.analytics.mn --draft` piped through the documented edit flow; `webnav dev graph-show --node progneo.analytics.mn` → eyeball states/affordances/shell. Restart `dev dashboard`, screenshot Sites tab (light AND dark), verify shell card + provisional badges render.
- [ ] **Step 4:** Commit: `test(acceptance): progneo 5-session rebuild pins the clean-map bar`
...
- [ ] **Step 4 — progneo confirm-visits:** if the `default` profile still holds the Cloudflare session: record a short session opening a SECOND report + SECOND dashboard (the `requests` the map asks for) → rebuild → previously-provisional states confirm and instance headings leave the cores. If auth expired: STOP and ask the user to re-login (gated).
