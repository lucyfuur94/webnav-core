...
**Goal:** Close X4/X5/X7/X8/X9 + X6-human, then prove capture completeness with real the analytics SPA recordings.
...
- the analytics SPA (Task 7 only): NEVER click Admin or Switch-to-Classic; headless; one browser; serial; reap after; real db is review-gated — no graph merge without passed review.
...
**Tests:** synthetic: single-shot alias NOT formed + request emitted; repeated pair formed; contradiction → needsFix; acceptance suite green (the analytics SPA aliases repeat across its sessions — verify and state the count in the report). Commit `fix(draft): aliases require reproduction; guarded redirects surface as needsFix (X4)`.
...
### Task 8: The analytics SPA validation (the goal-1 proof)
