...
describe('COMMANDS registry', () => {
  it('has all the registered verbs', () => {
    ...
    expect(names).toEqual(['capture', 'click', 'creds', 'dashboard', 'edge-add', 'effects', 'eval', 'export-map', 'go-back', 'graph-analyse', 'graph-edit', 'graph-show', 'import-map', 'list', 'login', 'mcp', 'mermaid', 'navigate', 'network', 'node-add', 'node-clear', 'node-rm', 'outline', 'read', 'record-start', 'record-stop', 'reload', 'search', 'sessions', 'snapshot', 'type', 'verify', 'walk', 'walk-resume']);
  });
