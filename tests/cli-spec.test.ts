...
describe('COMMANDS registry', () => {
  it('has all the registered verbs', () => {
    ...
    expect(names).toEqual(['capture', 'click', 'close', 'creds', 'dashboard', 'edge-add', 'effects', 'eval', 'export-map', 'go-back', 'graph-analyse', 'graph-edit', 'graph-show', 'import-map', 'ingest', 'list', 'login', 'mcp', 'mermaid', 'navigate', 'network', 'node-add', 'node-clear', 'node-rm', 'outline', 'read', 'record-live', 'record-start', 'record-stop', 'reload', 'search', 'session', 'sessions', 'snapshot', 'type', 'verify', 'walk', 'walk-resume']);
  });
