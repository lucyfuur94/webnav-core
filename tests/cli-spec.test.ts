...
describe('COMMANDS registry', () => {
  it('has all the registered verbs', () => {
    ...
    expect(names).toEqual(['capture', 'capture-loop', 'click', 'close', 'creds', 'dashboard', 'edge-add', 'effects', 'eval', 'export-map', 'go-back', 'graph-analyse', 'graph-edit', 'graph-show', 'import-map', 'ingest', 'list', 'login', 'mcp', 'mermaid', 'navigate', 'network', 'node-add', 'node-clear', 'node-rm', 'outline', 'pattern-propose', 'profile-status', 'read', 'record-live', 'record-rename', 'record-start', 'record-stop', 'reload', 'review', 'search', 'session', 'sessions', 'snapshot', 'type', 'verify', 'walk', 'walk-resume']);
  });
