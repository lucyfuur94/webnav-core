...
// ── X10 worked example — the real progneo div-soup date-picker (extension-loop plan Task 3) ──
// Sourced from `graph-analyse --draft` on the real 5-session progneo recording: clicking the
...
// of detection).
describe('grammar: X10 div-soup date-picker (real progneo shape) — core pack flips opener to reveal, days never stored', () => {
  const REPORT = [
    '- heading "Report" [ref=e1]',
    '- button "Last 7 Days (CD) : 02 Jul 2026 - 08 Jul 2026UTC" [ref=e2]',
    '- button "Add filter" [ref=e3]',
    '- button "Share" [ref=e4]',
    '- paragraph "OS and Device Report" [ref=e5]',
    '- paragraph "Table" [ref=e6]',
    '- paragraph "Flat" [ref=e7]',
    '- paragraph "Draft" [ref=e8]',
  ].join('\n');
  // The real shape (from the actual progneo capture): a dialog root of nested role-less `generic`
  ...
  const enterReport = (): StoredActionEffect => ({
    seq: 0, capturedAt: 0, fromUrl: `${B}/auth/login`, fromSnapshot: AUTH,
    action: { role: 'button', name: 'Login', ref: 'e4', elementFp: { role: 'button', name: 'Login', near: null } },
    toUrl: `${B}/report/16116`, toSnapshot: REPORT, navigated: true, diff: { added: [], removed: [] } as any,
  });
  const openPicker: StoredActionEffect = {
    seq: 1, capturedAt: 0, fromUrl: `${B}/report/16116`, fromSnapshot: REPORT,
    ...
    toUrl: `${B}/report/16116`, toSnapshot: REPORT_WITH_PICKER, navigated: false,
