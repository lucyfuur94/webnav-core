...
describe('inferUrlModel', () => {
  ...
  it('a VARYING opaque-id base position is a {param}, not a locked literal (two tenants merge)', () => {
    ...
    // the ≥80% greedy peel, so /v3/1045/report/list kept its tenant and never merged with 1041's.)
    const urls = [
      ...
      'https://x.test/v3/1041/report/list', 'https://x.test/v3/1041/dashboard/list',
      'https://x.test/v3/1041/download/list', 'https://x.test/v3/1041/help-center',
      'https://x.test/v3/1041/announcements', 'https://x.test/v3/1041/report/list',
      // second tenant (1045), minority — same modules.
      'https://x.test/v3/1045/report/list', 'https://x.test/v3/1045/dashboard/list',
    ];
    const m = inferUrlModel(urls);
    expect(m.base).toEqual(['v3', '{param}']);                  // v3 constant, tenant position = param
    expect(m.keyOf('https://x.test/v3/1041/report/list')).toBe('/{param}/report/list');
    expect(m.keyOf('https://x.test/v3/1045/report/list')).toBe('/{param}/report/list');   // SAME key → merge
    expect(m.keyOf('https://x.test/v3/1045/dashboard/list')).toBe('/{param}/dashboard/list');
  });
