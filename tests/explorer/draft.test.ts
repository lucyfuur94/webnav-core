...
const shell = (activeHeading: string, extra: string[] = []): string => [
  ...
  '- link "Home" [ref=e2]:\n    - /url: https://x.test/v3/1041/home',
  '- link "Reports" [ref=e3]:\n    - /url: https://x.test/v3/1041/report/list',
  '- link "Dashboards" [ref=e4]:\n    - /url: https://x.test/v3/1041/dashboard/list',
  '- link "Announcements" [ref=e5]:\n    - /url: https://x.test/v3/1041/announcements',
  '- link "Help Center" [ref=e6]:\n    - /url: https://x.test/v3/1041/help-center',
  ...extra,
].join('\n');
...
describe('draftFromEffects — observation-based identity', () => {
  ...
  // RULE 1 — keying + alias: a pre-redirect ghost URL (missing the /v3/1041 tenant) is aliased
  // to the settled key via requestedUrl, so /report/list (ghost) and /v3/1041/report/list merge.
  ...
  // templated page; /dashboard/list vs /dashboard/1210 (different faces) must NOT merge.
  it('merges same-structure param URLs (report/{id}) and does NOT merge dashboard/list vs dashboard/1210', () => {
    ...
    const g = draftFromEffects([
      nav(`${XB}/report/16116/a`, REP_A),
      nav(`${XB}/report/16116/b`, REP_B),
      nav(`${XB}/report/16116/c`, REP_C),
      nav(`${XB}/dashboard/list`, DASH_LIST),
      nav(`${XB}/dashboard/1210`, DASH_ONE),
    ] as never);
    const labels = g.states.map((s) => s.label);
    // the three report variants collapsed into ONE templated page. Label = the MEANINGFUL tail
    // (opaque id segments `16116`/`{param}` dropped) → `report`, never a raw record id.
    expect(labels.filter((l) => l === 'report').length).toBe(1);
    expect(g.states.some((s) => s.urlPattern.includes('/report/16116/'))).toBe(true);
    // dashboard/list and dashboard/1210 did NOT merge (distinct faces). list stays a state; the
    // single-instance /dashboard/1210 (heading-only `Sales KPIs` fingerprint on an opaque-id tail)
...
describe('draftFromEffects — core-derived fingerprints (Task 10 rule 1)', () => {
  ...
  // DIFFERENT big heading (a per-instance title: "Testuser dashboard" vs "Q3 Board"). The
  ...
  it('a big heading that differs per instance is NOT in the fingerprint (no instance-data identity)', () => {
    ...
    const g = draftFromEffects([
      nav(`${XB}/dashboard/7`, dashLanding('Testuser dashboard')),
    ...
    expect(dash.fingerprint.join()).not.toContain('Testuser');
...
// ── Task 15: offline-acceptance findings, each reproduced synthetically (the real progneo data
...
describe('draftFromEffects — Task 15 acceptance findings (synthetic repros)', () => {
  it('a pre-redirect 404 sharing a merged key does NOT suffix the healthy sibling (report-list, not report-list-reports)', () => {
    ...
    // `/v3/1041/report/list` (real Reports). The 404 cluster must be held out to needsFix WITHOUT
    ...
    const GHOST404 = ['- heading "Page not found" [ref=e1]', '- paragraph "This page could not be found." [ref=e2]',
      '- link "Home" [ref=e3]:\n    - /url: https://x.test/v3/1041/home', '- link "Back" [ref=e4]',
  ...
  it('opaque id tail segments are dropped from labels (/report/16116/{hash} → report, colliding vizzes split by tab)', () => {
    // two structurally-DISTINCT vizzes of report 16116 (jaccard < 0.5 → dispose rejects the
    ...
    const g = draftFromEffects([
      nav(`${XB}/report/16116/aaaaaaaaaaaaaaaa`, VIZ_A),
      nav(`${XB}/report/16116/bbbbbbbbbbbbbbbb`, VIZ_FLAT),
      nav(`${XB}/announcements`, shell('Announcements', ['- button "Post" [ref=e7]', '- paragraph "News" [ref=e8]'])),
      nav(`${XB}/help-center`, shell('Help Center', ['- textbox "Ask" [ref=e7]', '- button "Contact" [ref=e8]'])),
    ] as never);
    const labels = g.states.map((s) => s.label);
    // both collide on the clean base `report` (opaque `16116`+hash dropped); the resolver
    ...
    expect(labels.join(',')).not.toMatch(/16116|aaaaaaaa|bbbbbbbb/);   // no raw id in any label
  });
  ...
  it('a folded per-chip name never anchors identity (report fp has no OS Remove / eCPM Remove)', () => {
    ...
    const g = draftFromEffects([
      nav(`${XB}/report/16116/aaaaaaaaaaaaaaaa`, core(['- textbox "Search rows" [ref=e14]'])),
      nav(`${XB}/report/16116/aaaaaaaaaaaaaaaa`, core(['- textbox "Search rows" [ref=e14]'])),   // 2 landings → confirmed core
      nav(`${XB}/announcements`, shell('Announcements', ['- button "Post" [ref=e7]', '- paragraph "News" [ref=e8]'])),
      nav(`${XB}/help-center`, shell('Help Center', ['- textbox "Ask" [ref=e7]', '- button "Contact" [ref=e8]'])),
    ] as never);
  ...
  it('render-skew: mid-query + settled landings of ONE report merge (no subset-fp walk-ambiguous pair)', () => {
    // LIVE progneo defect (2026-07-12): /report/16116/{c8c6…} was captured MID-QUERY (sparse,
    // "Running query" Cancel button, metric chips still UNFOLDED) and /report/16116/{13fe…}
    // SETTLED (full folded data grid). Both are the SAME report 16116 → one `/report/{param}`
    ...
    const MIDQUERY = shell('OS and Device Report', ['- tab "Table" [ref=e7]', '- tab "Charts" [ref=e8]',
    ...
    const SETTLED = shell('OS and Device Report', ['- tab "Table" [ref=e7]', '- tab "Charts" [ref=e8]',
    ...
    const g = draftFromEffects([
      nav(`${XB}/report/16116/c8c6c374dd0f529fa815516679436fef`, MIDQUERY),
      nav(`${XB}/report/16116/13fe32057e11e4069fecf2e727a2d7d9`, SETTLED),
      nav(`${XB}/announcements`, shell('Announcements', ['- button "Post" [ref=e7]', '- paragraph "News" [ref=e8]'])),
      nav(`${XB}/help-center`, shell('Help Center', ['- textbox "Ask" [ref=e7]', '- button "Contact" [ref=e8]'])),
    ] as never);
    const reportStates = g.states.filter((s) => /\/report\/16116\//.test(s.urlPattern));
  ...
  it('urlPattern = most-observed settled landing URL, not the first-observed ghost (session ordering)', () => {
    ...
    // inserted the tenant segment yet): /v3/report/list vs /v3/1041/report/list — inferUrlModel's
  ...
  it('single-instance param page with a heading-only fingerprint is held out (no instance-data identity)', () => {
    ...
    const g = draftFromEffects([
      nav(`${XB}/dashboard/1210`, shell('Testuser', ['- img "avatar" [ref=e7]', '- paragraph "welcome" [ref=e8]'])),
      nav(`${XB}/announcements`, shell('Announcements', ['- button "Post" [ref=e7]', '- paragraph "News" [ref=e8]'])),
      nav(`${XB}/help-center`, shell('Help Center', ['- textbox "Ask" [ref=e7]', '- button "Contact" [ref=e8]'])),
      nav(`${XB}/download/list`, shell('Downloads', ['- button "All" [ref=e7]', '- listitem "a.csv" [ref=e8]'])),
    ] as never);
    expect(g.states.every((s) => !s.fingerprint.join().includes('Testuser'))).toBe(true);
    expect((g.needsFix ?? []).some((n) => /instance data/i.test(n.reason) && n.urlPattern.includes('/dashboard/1210'))).toBe(true);
  });
});
...
describe('draftFromEffects — identity-face normalization (subtree widgets)', () => {
  const NB = 'https://n.test/v3/1041';
  const bar = (h: string, extra: string[] = []): string => [
    `- heading "${h}" [ref=e1]`,
    '- link "Home" [ref=e2]:\n    - /url: https://n.test/v3/1041/home',
    '- link "Reports" [ref=e3]:\n    - /url: https://n.test/v3/1041/report/list',
    '- link "Dashboards" [ref=e4]:\n    - /url: https://n.test/v3/1041/dashboard/list',
    '- link "Announcements" [ref=e5]:\n    - /url: https://n.test/v3/1041/announcements',
    '- link "Help Center" [ref=e6]:\n    - /url: https://n.test/v3/1041/help-center',
    ...extra,
  ].join('\n');
  ...
  it('MERGE: two /dashboard/{id} instances differing only by widget instances fold to ONE state', () => {
    const g = draftFromEffects([
      nnav(`${NB}/dashboard/1210`, dashboard('Sales', 'Traffic', 'Signups')),
      nnav(`${NB}/dashboard/1215`, dashboard('Revenue', 'Users', 'Churn')),
