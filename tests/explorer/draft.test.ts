...
// are stripped, so /report/list?currentTab=owned and /report/16116/<hash> collapse correctly.
...
describe('draftFromEffects — stable-pathname keying (one page, not many)', () => {
  it('stablePathKey strips query, hash, and trailing id/hash segments', () => {
    expect(stablePathKey('https://x.test/v3/1041/report/list?currentTab=owned')).toBe('/v3/1041/report/list');
    expect(stablePathKey('https://x.test/v3/1041/report/16116/bd5a1a4a6352048e18d747c7e95ce3a0?source=listing')).toBe('/v3/1041/report');
    expect(stablePathKey('https://x.test/v3/1041/dashboard/1210')).toBe('/v3/1041/dashboard');
    expect(stablePathKey('https://x.test/v3/1041/report/draft')).toBe('/v3/1041/report/draft'); // non-id trailing kept
  });
