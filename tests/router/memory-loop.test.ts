...
describe('memory loop (cross-run, file-backed MapStore, no browser)', () => {
  ...
  it('builds the skeleton once on run-1 and never re-explores on run-2 (criterion #3)', () => {
    const store = new MapStore(DB_PATH);
    const spy = vi.spyOn(skeleton, 'exploreGitHub');

    // RUN 1 — empty store: recallViaMap must build the skeleton exactly once.
    ...
    // Skeleton states are now persisted in the (file-backed) store.
    ...
    // RUN 2 — a FRESH MapStore reopened from the SAME file (simulating a separate
    // `webnav recall` invocation). The skeleton must come from DISK, not an
    // in-memory object, so exploreGitHub must NOT be called. This is the true
    // cross-run memory guarantee (criterion #3), not mere object reuse.
    spy.mockClear();
