...
// #3a — a map recorded on one account path (.../v3/1041/report/list) must still be
// RECALLABLE when the user browses the same page under a DIFFERENT account id
// (.../v3/1033/report/list). Volatile numeric/opaque id path segments are NOT page
...
const reportList = makeState({
  ...
  urlPattern: 'https://site.example/v3/1041/report/list',
...
const dashboardList = makeState({
  ...
  urlPattern: 'https://site.example/v3/1041/dashboard/list',
...
const reportDraftTable = makeState({
  ...
  urlPattern: 'https://site.example/v3/1041/report/draft/71336cdec7f7af8948dfb95faf37ad26',
...
describe('recall across account ids (#3a — fingerprint identity, url is an attribute)', () => {
  it('matches the same page under a DIFFERENT account id (1041-recorded -> 1033 live)', () => {
    // The live url is /v3/1033/report/list — a different account segment than the
    // recorded /v3/1041/... — but the page structure is identical, so recall must match.
