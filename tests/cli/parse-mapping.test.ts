...
describe('parseArgs — mapping verbs (under dev)', () => {
  ...
  it('parses profile-status with --url override', () => {
    expect(parseArgs(['dev', 'profile-status', '--profile', 'work', '--site', 'app.example.com', '--url', 'https://app.example.com/dashboard']))
      .toEqual({ cmd: 'profile-status', profile: 'work', site: 'app.example.com', url: 'https://app.example.com/dashboard' });
  });
