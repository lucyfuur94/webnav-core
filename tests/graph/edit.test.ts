...
describe('editGraph — full typed affordance authoring', () => {
  ...
  it('authors affordances on a state, core on an edge, and creates the node row', () => {
    const store = MapStore.fromDatabase(new Database(':memory:'));
    editGraph(store, 'shop.example', {
    ...
    // Editing a graph creates/keeps the site-node row (id + derived home url).
    expect(store.getNode('shop.example')!.homeUrl).toBe('https://shop.example');
  });
...
it('threads urlPattern template into the store (viewers show /dashboard/{param})', () => {
  ...
  editGraph(store, 'app.example', {
    states: [
      { label: 'dashboard', urlPattern: 'https://app.example/dashboard/8001',
  ...
  expect(s.urlPattern).toBe('https://app.example/dashboard/8001');   // instance URL kept as-is
