...
describe('reveal sub-node synthesis (viewer-only)', () => {
  it('materialises a sub-node + a parent→sub reveal edge, child edges leave the sub-node', () => {
    ...
    const { subStates, revealEdges, childOwner } = synthesizeRevealSubNodes(states);
    ...
    const le = buildLayoutEdges(edges, revealEdges, childOwner, F);
  ...
  it('parent node no longer counts reveal children in its badge/height estimate', () => {
    const { subStates } = synthesizeRevealSubNodes([inventoryState()]);
  ...
  it('recurses nested overlays (a reveal child that itself has children)', () => {
    ...
    const { subStates, revealEdges } = synthesizeRevealSubNodes(states);
